...
def create_cache_tables():
    ...
    try:
        ...
        with db.cursor() as cursor:
            # 创建 aliyun_ecs_cache 表
            logger.info("Creating aliyun_ecs_cache table...")
            cursor.execute("""
            ...
            # 创建 aliyun_sync_status 表
            logger.info("Creating aliyun_sync_status table...")
            cursor.execute("""
            ...
            # 创建 aliyun_instance_config 表
            logger.info("Creating aliyun_instance_config table...")
            cursor.execute("""
            CREATE TABLE IF NOT EXISTS `aliyun_instance_config` (
              ...
              PRIMARY KEY (`id`),
              UNIQUE KEY `unique_instance` (`instance_id`)
            ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='阿里云ECS实例连接配置'
            """)
            
            # 插入初始同步状态记录
            logger.info("Inserting initial sync status records...")
            cursor.execute("""
        ...
        # 提交更改
        db.commit()
        logger.info("All cache tables created successfully!")
